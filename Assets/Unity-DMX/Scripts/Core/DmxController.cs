...
public class DmxController : MonoBehaviour
{
    ...
    //[SerializeField] 
    ArtNetDmxPacket latestReceivedDMX;
    //[SerializeField] 
    ArtNetDmxPacket dmxToSend;
    byte[] _dmxData;
    ...
    private void Awake()
    {
        ...
        if (isServer) artnet.Open(FindFromHostName("localhost"), null);
        //サブネットマスクを設定すると、自分に送らないアドレスを設定してくれる（便利！）
        //なのだが、デバッグがめんどくさくなる
        if (dmxToSend is null) dmxToSend = new ArtNetDmxPacket();
